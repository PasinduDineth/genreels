...
const normalizeOllamaResponse = (data: { response?: string; message?: { content?: string } }) => {
...
const requestTextViaRunpodLlm = async (systemPrompt: string, userPrompt: string) => {
  ...
  if (!response.ok) {
    ...
    throw new AppError(
      `Runpod LLM request failed with status ${response.status}${bodyText ? `: ${bodyText}` : ''}.`,
      500,
      'RUNPOD_LLM_REQUEST_FAILED',
    );
  }

  const payload = (await response.json()) as { response?: string; message?: { content?: string } };
  const content = normalizeOllamaResponse(payload);
  if (!content) {
    throw new AppError('Runpod LLM returned an empty text response.', 500, 'RUNPOD_LLM_EMPTY_RESPONSE');
  }
...
export const pollinationsClient = {
  ...
  async generatePromptText({ topic, narrative }: PromptGenerationRequest): Promise<string> {
    const userPrompt = `Topic: ${topic ?? 'Unknown true story'}\nNarrative: ${narrative ?? ''}`;
    return requestTextViaRunpodLlm(createPromptGeneratorSystemMessage(), userPrompt);
  },

  async generateSocialMetadataText({ feedback, narrative, topic }: SocialMetadataGenerationRequest): Promise<string> {
    const userPrompt = [
      `Topic: ${topic ?? 'Unknown true story'}`,
      `Narrative: ${narrative}`,
      feedback ? `Revision notes: ${feedback}` : null,
    ].filter(Boolean).join('\n');
    return requestTextViaRunpodLlm(createSocialMetadataSystemMessage(), userPrompt);
  },
};
