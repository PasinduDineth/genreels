...
touch "$STARTUP_LOG" "$OLLAMA_LOG" "$GATEWAY_LOG" "$TTS_LOG" "$IMAGE_LOG" "$VIDEO_LOG" \
      "$CONTROL_LOG" "$INSTALL_LOG" "$PREFLIGHT_LOG"
...
SECURITY:
  This gateway has NO authentication. Anyone with the public RunPod proxy URL
  can start services and consume GPU resources.
